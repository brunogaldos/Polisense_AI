import { TOPICS, DATA_TYPES, FREQUENCIES, TIME_PERIODS } from 'utils/concepts';
import { BASEMAPS, LABELS } from 'components/map/constants';
import { EXPLORE_SECTIONS } from 'layout/explore/constants';

export default {
  // Datasets
  datasets: {
    list: [],
    loading: true,
    error: null,
    page: 1,
    limit: 10,
    total: 0,
    selected: null,
  },
  filters: {
    open: false,
    search: '',
    tab: 'topics',
    tags: [], // Added tags array for search functionality
    options: {
      topics: TOPICS,
      data_types: DATA_TYPES,
      frequencies: FREQUENCIES,
      time_periods: TIME_PERIODS,
      custom: {
        label: 'Others',
        value: 'custom',
        list: [],
      },
    },
    selected: {
      topics: [],
      data_types: [],
      frequencies: [],
      time_periods: [],
      custom: [],
    },
  },
  sort: {
    selected: 'most-viewed',
    direction: -1,
    isSetFromDefaultState: true,
    options: [
      { value: 'updatedAt', label: 'Last modified' },
      { value: 'most-viewed', label: 'Most viewed' },
      { value: 'most-favorited', label: 'Most favorited' },
      { value: 'relevance', label: 'Relevance' },
      { value: 'createdAt', label: 'Date added' },
    ],
  },

  // Map
  map: {
    viewport: {
      zoom: 1.5,
      latitude: 20, // Arequipa, Peru latitude
      longitude: 0, // Arequipa, Peru longitude
      pitch: 0,
      bearing: 0,
      transitionDuration: 250,
    },
    bounds: {
      box: null,
      options: {},
    },
    basemap: BASEMAPS.terrain.id,
    labels: LABELS.dark.id,
    boundaries: true,
    layerGroups: [],
    geojsonLayers: [],
    layerGroupsInteraction: {},
    layerGroupsInteractionSelected: null,
    layerGroupsInteractionLatLng: null,
    drawer: {
      isDrawing: false,
      data: null,
    },
    // contains an area ID to display in the map
    aoi: null,
    // contains a geostore id to preview in the map
    previewAoi: null,
    // contains params to be modified in the layerSpec of every layer
    // 'layer-id': {
    //  'key-to-modify: {
    //    startDate: '09-09-2018',
    //    endDate: '12-31-2019',
    //    zoom: 4,
    //    ...
    //  }
    // }
    //
    parametrization: {},
  },

  // Sidebar
  sidebar: {
    open: false,
    anchor: null,
    section: EXPLORE_SECTIONS.ALL_DATA, // Changed from DISCOVER to ALL_DATA as default
    subsection: null,
    selected: null,
    selectedCollection: null,
  },

  tags: {
    tooltip: false,
    list: [],
    loading: false,
    error: null,
  },
};
