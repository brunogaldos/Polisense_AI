...
function checkBasicAuth(credentials) {
  return function authMiddleware(req, res, nextAction) {
    if (
      ...
      // Note: Authentication is now handled client-side via Firebase Auth (no API routes needed)
    ...
    if (process.env.SERVER_ONLY !== 'true') {
      app.prepare().then(() => {
        ...
        const httpListener = server.listen(port, (err) => {
          ...
          console.info(
            `> Ready on http://localhost:${port}/data/explore [RW_ENV: ${process.env.NEXT_PUBLIC_RW_ENV || 'development'}] [NODE_ENV: ${process.env.NODE_ENV}]`,
          );
