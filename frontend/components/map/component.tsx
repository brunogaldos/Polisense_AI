...
export const Map = ({
  ...
  const handleBoundaries = useCallback(
    (boundaries: boolean) => {
      ...
      boundariesLayers.forEach((l) => {
        ...
        // Change boundary color to black when visible
        if (boundaries && l.type === 'line') {
          map.setPaintProperty(l.id, 'line-color', '#000000');
        }
      });
    },
    [mapRef],
  ...
  const mapboxToken = process.env.NEXT_PUBLIC_RW_MAPBOX_API_TOKEN;
  const hasMapboxToken = !!mapboxToken;

  // Suppress Mapbox errors when token is not available
  useEffect(() => {
    if (!hasMapboxToken) {
      // Suppress console errors for Mapbox API calls when token is missing
      const originalError = console.error;
      console.error = (...args) => {
        const message = args[0]?.toString() || '';
        if (message.includes('mapbox') || message.includes('Mapbox')) {
          return; // Suppress Mapbox-related errors
        }
      ...
      {hasMapboxToken ? (
        ...
        <div className="flex items-center justify-center w-full h-full bg-gray-100 text-gray-500">
          <div className="text-center">
            ...
            <div className="text-sm">Mapbox API token not configured</div>
          </div>
        </div>
      )}
    </div>
