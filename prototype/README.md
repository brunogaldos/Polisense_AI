...
- **`frontend/apps/ingest_app.py`** — parse papers, manage Weaviate ingestion, query via chat
- **`frontend/apps/dashboard_chat.py`** — main Atlas dashboard with 5-tab analytics and RAG chatbot
- **`frontend/dashboard_viewer_graph.py`** — attribute analytics and full-text traceability viewer
...
└────────────────────────┬────────────────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  STAGE 1 — OCR + PARSING          backend/pipeline/ocr.py               │
│                                                                         │
...
└────────────────────────┬────────────────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  STAGE 2 — MARKDOWN + TABLE EXPORT    backend/pipeline/chunking.py      │
│                                                                         │
...
└────────────────────────┬────────────────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  STAGE 3 — CHUNKING + METADATA ENRICHMENT    backend/pipeline/chunking.py│
│                                                                         │
...
└────────────────────────┬────────────────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  STAGE 4 — EMBEDDING + WEAVIATE INGESTION    backend/pipeline/weaviate/ │
│                                                                         │
...
└────────────────────────┬────────────────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  STAGE 5 — RETRIEVAL    backend/pipeline/weaviate/retrieve.py           │
│                                                                         │
...
└────────────────────────┬────────────────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  STAGE 6 — FRONTEND    frontend/apps/                                   │
│                                                                         │
│  frontend/apps/ingest_app.py                                            │
│  ├─ Ingest tab      → parse, rotate tables, chunk papers                │
│  ├─ Documents tab   → ingest chunks to Weaviate                         │
│  └─ Chat tab        → hybrid RAG query interface                        │
│                                                                         │
│  frontend/apps/dashboard_chat.py                                        │
│  └─ 5-tab Atlas dashboard + RAG chatbot with evidence table             │
│                                                                         │
...
│
├── materials/
│   └── articles/                  ← Input PDFs (shared data source)
│       ├── Baloch_2022.pdf
│       ├── Seymour_2016.pdf
│       └── ...
│
...
│           └── {paper_id}_fig0.png
│
├── backend/                       ← Backend: RAG pipeline + services
...
│   │   ├── extract.py             ← LLM evidence extraction (OpenAI/OpenRouter)
...
│   │   ├── run_extraction.py
│   │   ├── tablejsonviewer.py
│   │   ├── graph_viewer.py        ← NetworkX graph viewer (Streamlit)
│   │   └── ...
│   ├── docker-compose.yml         ← Weaviate container
│   └── requirements.txt           ← All dependencies
│
├── frontend/                      ← Frontend: Streamlit UIs
│   ├── apps/                      ← Streamlit applications
│   │   ├── dashboard_chat.py      ← Main Atlas dashboard + RAG chatbot
│   │   ├── dashboard.py           ← Attribute analytics dashboard
│   │   ├── ingest_app.py          ← Ingest + Documents + Chat
│   │   └── extract.py             ← Evidence extraction CLI
│   ├── dashboard_viewer_graph.py  ← Sepsis Global Attribute Atlas viewer
│   ├── graph_viewer.py            ← NetworkX interactive graph viewer
│   └── chatbot_panel_layout.md   ← Chat UI layout spec
│
├── demo/
│   └── frontToken_burners.gif
...
### Stage 3 — Chunking (`backend/pipeline/chunking.py`)
...
Weaviate runs at `http://localhost:8090`. Data is persisted in the `weaviate_data` Docker volume.
...
```bash
streamlit run frontend/apps/ingest_app.py
```
...
### Atlas Dashboard + RAG Chatbot

```bash
streamlit run frontend/apps/dashboard_chat.py
```

### Attribute Analytics Dashboard

```bash
streamlit run frontend/apps/dashboard.py
```

### Attribute Viewer

```bash
streamlit run frontend/dashboard_viewer_graph.py
```
...
All pipeline modules are run from the `backend/` directory with `-m`:
...
All paths and service settings are defined in `backend/pipeline/config.py` and can be overridden with environment variables:

```python
# Paths (derived from repo root — always absolute)
REPO_ROOT        = Path(__file__).resolve().parent.parent.parent  # backend/pipeline/ → root
...
- **CPU-only inference** — Both embedder and reranker run on CPU by default. Set `device="cuda"` in `backend/pipeline/weaviate/embedder.py` and `reranker.py` to use GPU.
