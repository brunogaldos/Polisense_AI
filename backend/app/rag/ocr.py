"""PDF → ParsedPaper via Docling. Ported from backend-mini.

Layout-aware parse (sections with page numbers, tables as Markdown, figures
cropped to PNG). Results are cached as {stem}.json under app.rag.config.cache_dir().

The Docling converter is built lazily (it loads model weights) so importing this
module in the web process is cheap; the converter is only constructed the first
time parse_pdf() runs.
"""
...
from app.rag.config import cache_dir, figure_dir
